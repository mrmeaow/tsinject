# tsinject vs tsyringe vs InversifyJS – Competitive Comparison

This document explains why **tsinject** was built, how it differs from the two most popular TypeScript DI libraries, and what advantages it brings to developers who need a modern, type‑safe, and performance‑focused container.

---

## 1. Core Philosophy

| Aspect | tsinject | tsyringe | InversifyJS |
|--------|----------|----------|-------------|
...
| **Optional reflect‑metadata bridge** | Core never requires `reflect-metadata`. The optional `tsinject/reflect` sub‑export can be imported for a drop‑in “auto‑inject” experience. | Requires `reflect-metadata` for *any* injection of concrete classes. |

---

## 2. API Surface Size

| Feature | tsinject | tsyringe | InversifyJS |
|---------|----------|----------|------------|
...
- **Async factories** – `tsinject` separates sync (`resolve`) and async (`resolveAsync`) APIs, throwing a clear `AsyncFactoryError` when a sync call is made on an async provider. 
...
**Error classes** in tsinject (`CircularDependencyError`, `ResolutionError`, `AsyncFactoryError`, `DisposedContainerError`) include:
- Token name
- Dependency chain
...
| Metric | tsinject | tsyringe | InversifyJS |
|--------|----------|----------|-------------|
...
Benchmarks are illustrative; real‑world performance will depend on the number of registered providers and scope depth, but tsinject’s lean core consistently shows lower overhead.

---

## 5. Scoping Model

- **tsinject** – Explicit hierarchical scopes via `container.createScope(name)`. Scoped lifetimes are cached in the child container; singletons are always cached in the root. This mirrors the classic DI container model while keeping the API tiny.
...
| Area | tsinject | tsyringe | InversifyJS |
|------|----------|----------|------------|
...
| Test utilities | `tsinject` works out‑of‑the‑box with Vitest, Jest, etc.; the container is easy to mock because it is just a class. | No dedicated testing helpers; users reset the global container manually. | Requires `inversify‑mock‑container` or similar third‑party helpers. |

---

## 7. Why Choose tsinject?
...
| From | To | Key actions |
|------|----|-------------|
| **tsyringe** | **tsinject** | 1️⃣ Create `Token<T>` for every interface. 2️⃣ Replace global `container` with `new Container()`. 3️⃣ Add explicit `@inject(Token)` where needed. 4️⃣ Optionally import `tsinject/reflect` for auto‑inject fallback. |
| **InversifyJS** | **tsinject** | 1️⃣ Replace symbols with `createToken`. 2️⃣ Remove `container.bind` fluent API; use `registerClass`/`registerFactory`. 3️⃣ Drop `reflect-metadata` requirement – add explicit `@inject`. 4️⃣ Use `container.createScope` for request‑scoped lifetimes. |
...
| **Optional reflect bridge** | ✅ (`tsinject/reflect`) | ✅ (required) | ✅ (required) |

---

**Conclusion** – If you need a lightweight, type‑first DI container that works cleanly with modern TypeScript, provides clear async handling, and avoids hidden globals, **tsinject** offers a superior developer experience compared to both **tsyringe** and **InversifyJS**. Its design deliberately focuses on compile‑time safety, minimal runtime overhead, and straightforward module composition, positioning it as a compelling alternative for new projects and for teams looking to modernize existing codebases.
