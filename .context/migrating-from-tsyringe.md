# Migrating from **tsyringe** to **tsneedle**

Both libraries provide decorator‑based dependency injection for TypeScript, but **tsneedle** makes stricter type guarantees and removes the implicit global container.

---

## Core differences

| Feature | tsyringe | tsneedle |
|---------|----------|----------|
...
| **Reflect‑metadata** | Required (`emitDecoratorMetadata` + `reflect-metadata` import) for automatic parameter type inference. | Never required in the core path – `tsneedle` works without any runtime metadata. An optional `tsneedle/reflect` sub‑export can be imported to enable the same behaviour if you want a drop‑in migration path. |
...
3. **Update decorators**
   ...
   - If you previously relied on `reflect-metadata` for automatic injection, either import `tsneedle/reflect` (opt‑in bridge) **or** add explicit `@inject` annotations.
4. **Adjust lifecycles**
   ```ts
   // tsyringe singleton
   container.registerSingleton(ILogger, ConsoleLogger);
   // tsneedle singleton via decorator or option
   @singleton()
...
6. **Update tests**
   Ensure tests instantiate a fresh `Container` instead of using the global one. This improves isolation and mirrors tsneedle’s design.
7. **Optional: enable reflect bridge**
   For a quick win, add `import 'tsneedle/reflect';` at the entry point. This will make `@inject` optional for concrete classes, mimicking tsyringe’s behaviour while still keeping the core metadata‑free.
